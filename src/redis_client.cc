    ...
    while (true)
    {
        ...
        if (redisGetReply(_context, (void **)&_reply) == REDIS_OK)
        {
            ...
            else if (_reply->element[2]->str)
            {
                ...
                _Callback(_reply->element[2]->str);
            }
        }
        else
        {
...
void RedisClient::_Callback(const char *data)
{
    Operation operation;
    Response response;
    ...
    _ResponseToRedis(&response, operation.channel());
}

void RedisClient::_CallbackAddUser(const Operation *operation, Response *response)
{
...
void RedisClient::_CallbackRemoveUser(const redis::Operation *operation, redis::Response *response)
{
...
void RedisClient::_CallbackGetStats(const redis::Operation *operation, redis::Response *response)
{
...
void RedisClient::_CallbackGetAllStats(GetStatsOperationResponse *get_stats_operation_response)
{
...
void RedisClient::_CallbackGetUserOrInoundStats(GetStatsOperationResponse *get_stats_operation_response, const string &user_or_inbound)
{
    // 查询
    QueryStatsResponse query_stats_response;
    RPCStatsService(&query_stats_response);
...
bool RedisClient::_ResponseToRedis(Response *response, const string &channel)
{
    string cmd = "PUBLISH " + channel + response->SerializeAsString();
    return _ExcuteCommand(cmd.c_str());
}

bool RedisClient::_UpdateConfig(const string &config, const AddUserOperation &add_user_operation)
{
    Document root;
...
bool RedisClient::_UpdateConfig(const string &config, const RemoveUserOperation &remove_user_operation)
{
    Document root;
    if (!ReadJson(config.c_str(), &root))
    {
