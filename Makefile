...
PROTOS = proxy/vmess/account proxy/vless/account \
		 ...
		 app/stats/command/command
		 
OTHER_OBJS = client/handler_service_client.o client/stats_service_client.o

GRPC_CC = app/proxyman/command/command app/stats/command/command
OBJS_PATH = objs
OBJS = $(addprefix $(OBJS_PATH)/,main.o $(addsuffix .pb.o,$(PROTOS)) $(addsuffix .grpc.pb.o,$(GRPC_CC)) ) $(addprefix $(OBJS_PATH)/,$(OTHER_OBJS)) 
...
.PHONY: cp
cp:
	cd $(PROTO_SRC) && cp --parents $(addsuffix .proto,$(PROTOS)) /root/grpc_proto/v2ray_man/v2ray_manage_client
...
.PHONY: clean
clean:
	rm -rf app common proxy transport config.* $(OBJS_PATH)/*
	rm -f demo
